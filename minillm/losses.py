...
class Loss():
    ...
    def ft_loss(self,batch, KN):
        batch_query = batch.query_tensors
        empty_tensor = torch.empty((batch_query.size(0), 0), dtype=batch_query.dtype).to(self.trainer.device)
        query_ids = batch_query
        batch = self.trainer.get_model_inputs(batch_query, empty_tensor)
    
        mode = "teacher"
        gen_out = self.trainer.generate(**batch, return_dict_in_generate=True, mode=mode, output_scores=True)
        ...
        model_batch = self.trainer.get_model_inputs(query_ids, response_ids)
        if self.trainer.teacher_model is not None:            
            # scores = KN.get_scores(self.trainer.tokenizer, model_batch, query_ids, inf_mask, response_ids, 1, 8, "integrated_grads", True)
            # scores_mlp, scores_hidd = KN.get_scores(self.trainer.tokenizer, model_batch, query_ids, inf_mask, response_ids, 1, 8, self.args.method, True)
            with torch.no_grad():
                    teacher_outputs = self.trainer.teacher_model(**model_batch, output_hidden_states=True, return_dict=True, use_cache=False)
            
            # chnage it to return list of scores based on each layer_index 
            import ast
            teacher_layer_indices = ast.literal_eval(self.args.teacher_layer_indices)
            if self.args.mlp == True and self.args.hidden == True:
                    scores_mlp, scores_hidd = KN.get_scores(self.trainer.tokenizer, model_batch, query_ids, inf_mask, response_ids, 1, 8, self.args.method, True)
                    important_mlp_features = []
                    important_hidden_features = []
                    for i in range(len(teacher_layer_indices)):
                        scores_mlp = scores_mlp[i].squeeze(0)
                        scores_hidd = scores_hidd[i].squeeze(0)
                        last_layer_mlp = self.trainer.teacher_model.transformer.h[teacher_layer_indices[i]].mlp  # 11
                        before_last_hidden_state = teacher_outputs.hidden_states[teacher_layer_indices[i]-1]  # 10
                        mlp_output = last_layer_mlp.c_fc(before_last_hidden_state) 
                        #K = 3072  # Choose how many top hidden units you want
                        top_k_indices_mlp = torch.topk(scores_mlp, self.args.student_mlp_size).indices
                        top_k_indices_hidd = torch.topk(scores_hidd, self.args.student_hidd_size).indices
                        important_mlp_features.append(mlp_output[:, :, top_k_indices_mlp])
                        important_hidden_features.append(teacher_outputs.hidden_states[self.args.layer_index][:, :, top_k_indices_hidd])
                    return important_mlp_features, important_hidden_features, model_batch
    
            if self.args.mlp == True and self.args.hidden == False:
                    
                    scores_mlp = KN.get_scores(self.trainer.tokenizer, model_batch, query_ids, inf_mask, response_ids, 1, 8, self.args.method, True)
                    important_mlp_features = []
                    for i in range(len(teacher_layer_indices)):
                        scores_mlp = scores_mlp[i].squeeze(0)
                        last_layer_mlp = self.trainer.teacher_model.transformer.h[teacher_layer_indices[i]].mlp  # 11
                        before_last_hidden_state = teacher_outputs.hidden_states[teacher_layer_indices[i]-1]  # 10
                        mlp_output = last_layer_mlp.c_fc(before_last_hidden_state) # will take all the tokens instead 
                        # mlp_output_newtokens = mlp_output[:,start:end,:]
                        #K = 3072  # Choose how many top hidden units you want
                        top_k_indices_mlp = torch.topk(scores_mlp, self.args.student_mlp_size).indices
                        important_mlp_features.append(mlp_output[:, :, top_k_indices_mlp])
                    return important_mlp_features, model_batch
            
    
            if self.args.hidden == True and self.args.mlp == False:
                    scores_hidd = KN.get_scores(self.trainer.tokenizer, model_batch, query_ids, inf_mask, response_ids, 1, 8, self.args.method, True)
                    important_hidden_features = []
                    for i in range(len(teacher_layer_indices)):
                        #scores_hidd = scores_hidd[i].squeeze(0).mean(dim=0).mean(dim=0)
                        scores_hidd_out = scores_hidd[i].squeeze(0).sum(dim=0).sum(dim=0)
                        top_k_indices_hidd = torch.topk(scores_hidd_out, self.args.student_hidd_size).indices
                        important_hidden_features.append(teacher_outputs.hidden_states[teacher_layer_indices[i]][:, :, top_k_indices_hidd])
                    return important_hidden_features, model_batch
