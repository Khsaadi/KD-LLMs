...
class PPOTrainer():
        ...
        #for training_epoch in range(1):
            ...
            #for ppo_epoch in range(1):
                for it, batch in enumerate(self.train_dataloader):
                    ...
                    # get the output and last layer of the teacher
                    # with torch.no_grad():
                    #     teacher_outputs = self.trainer.teacher_model(**input_batch, output_hidden_states=True, return_dict=True, use_cache=False)
                    #     teacher_logits = teacher_outputs.logits
                    # teacher_last_layer = teacher_outputs.hidden_states[-1]
                    # output_teacher = teacher_logits
                    # teacher_probs = F.softmax(output_teacher[:,-1, :], dim=-1)
                    # get the output and last layer of the student
                    # output_stude = ppo_logits
                    # probs = F.softmax(output_stude[:,-1, :], dim=-1)
                    # student_last_layer = outputs.hidden_states[-1]
                    # Compute gradients and retain the graph for further usage
                    # grad_stu = torch.autograd.grad(output_stude, student_last_layer, retain_graph=True)
                    # grad_teacher = torch.autograd.grad(torch.unbind(teacher_probs[:,]), student_last_layer)[0]
                    # print(grad_teacher)
                    # print(grad_student)
                    # Sum gradients over tokens to get the influence per hidden unit
                    # hidden_unit_influence = gradients.sum(dim=1)  # Shape: (batch_size, hidden_size)

                    # # Example: For a single batch, extract the key hidden units
                    # batch_idx = 0  # Use batch index 0 if you're processing a single input

                    # # Convert the tensor to absolute values (since we care about magnitude of influence)
                    # abs_influence = hidden_unit_influence[batch_idx].abs()

                    # # Get the indices of the top-K most influential hidden units
                    # K = 3072  # Choose how many top hidden units you want
                    # top_k_indices = torch.topk(abs_influence, K).indices
                    # important_features = teacher_last_layer[0, :, top_k_indices[0]]
                    # feature_loss = F.mse_loss(important_features, student_last_layer)
                    # # Display the top hidden units and their influence values
                    # print("Top-K Hidden Units and their Influence:")
                    # for idx in top_k_indices:
                    #     print(f"Hidden Unit {idx.item()} - Influence: {abs_influence[idx].item()}")
    ...
    def evaluate_ppo(self):  # noqa: C901
        ...
        all_rev_kl = []
        all_lens = []
        # maybe add interpretability loss here 
        table = []
